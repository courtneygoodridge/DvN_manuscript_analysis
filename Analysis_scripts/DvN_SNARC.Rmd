...
## Fully congruent vs partially congruent - Repeated Measures ANOVA for digits condition (Figures 7)
...
## Fully congruent vs partially congruent - linear regression and t-tests for digits condition (Figures 8)

```{r}
"computing dRT for full congruent"
drt.congruent.digits <- fully.partial.digits %>%
  dplyr::filter(Congruent.Full.Partial == "full") %>%
  dplyr::group_by(number, key_response, ppid) %>%
  dplyr::summarise(meanMS = mean(ms)) %>%
  dplyr::ungroup() %>%
...
#frequentist regression on mean drt values
congruent.n.mod <- lm(drt ~ number, data = drt.congruent.digits %>%
                        dplyr::group_by(number) %>%
                        dplyr::summarise(drt = mean(drt)))
...
## Numerosity - full vs partial congruency

```{r}
"Filtering numerosity data for fully vs partial congruent analysis."
