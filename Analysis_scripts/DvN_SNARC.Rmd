...
congruent.snarc <- ggplot() +
  geom_point(data = summary.n %>%
               dplyr::filter(Congruent == "Cong"), mapping = aes(x = magnitude, y = ms)) +
  geom_line(data = summary.n %>%
              dplyr::filter(Congruent == "Cong"), mapping = aes(x = magnitude, linetype = as.factor(hand), group = as.factor(hand), y = ms)) +
  geom_errorbar(data = summary.n %>%
                  dplyr::filter(Congruent == "Cong"), mapping = aes(x = magnitude, y = ms, ymin = ms - se, ymax = ms + se), width = 0.1) +
  scale_x_discrete(labels = c('Small','Large')) +
  scale_linetype_manual(name = "Response hand", labels = c("Left", "Right"),  values = c("solid", "dotted")) +
  ggtitle("A: Congruent") +
  ylim(450, 560) +
