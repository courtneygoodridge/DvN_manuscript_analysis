...
The following script aims to clean the data ready for analysis. The clean data file is available from the repository but this cleaning script is also provided for completeness.
...
library("Rmisc") # plotting interaction effects
library("BayesFactor") # computing bayes factor
library("emmeans")
library("gridExtra")
library("ez")
library("WebPower")
...
"Load data frames"
dat_missing <- read_excel("Session1.xlsx")
dat <- read_excel("Session2_combine.xlsx", 2)
```
...
```{r}
setwd("C:/Users/pscmgo/OneDrive for Business/PhD/Project/Experiment_Code/valter/Dots vs Numerosity")
write.csv(dat_main, file = "dat_main.csv")
```